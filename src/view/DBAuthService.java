...
public class DBAuthService implements AuthService {
    ...
    public DBAuthService(AppContext ctx) {
        this.ctx = ctx;
    }
    ...
    public Role login(String matricula, String senha) throws Exception {
        if (matricula == null || senha == null)
            throw new IllegalArgumentException("Informe matrícula e senha.");
        if (!matricula.matches("\\d{6}"))
            throw new IllegalArgumentException("Matrícula deve ter 6 dígitos.");
        if (!senha.matches("\\d{4}"))
            throw new IllegalArgumentException("Senha deve ter 4 dígitos.");

        try {
            // Checa diretamente nas duas tabelas
            if (checkSenha("FUNCIONARIO", matricula, senha))
                return Role.FUNCIONARIO;
            if (checkSenha("LEITOR", matricula, senha))
                return Role.ALUNO;
    ...
    /* ======= validações ======= */
    private static final java.util.regex.Pattern EMAIL_RE = java.util.regex.Pattern
